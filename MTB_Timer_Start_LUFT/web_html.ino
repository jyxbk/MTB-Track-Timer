...
String buildState() {
  ...
  for (uint8_t i = 0; i < stagCount; i++) {
    ...
    j += "{\"n\":\""; j += rn;
    j += "\",\"st\":"; j += (i < stagStarted) ? "true" : "false";
...
void sendHTML() {
  ...
  html += "<div class='mssel'>"
    ...
    "<button class='msbtn' id='ms-stag' onclick=\"_msw('stag',this)\">&#9201; Versetzt</button>"
    "</div>";
  ...
  // ── Versetzt-Inhalt ───────────────────────────────────────
  html += "<div id='msc-stag' style='display:none'>";
  if (stagDone) {
    ...
    html += "</table><a class='btn' href='/stagexit'>&#10006; Schlie&szlig;en</a></div>";
  } else if (stagMode) {
    html += "<div class='c' style='border-left:3px solid #64b5f6'>";
    html += "<div class='lb'>&#9201; Versetzt &mdash; " + String(stagStarted) + "/" + String(stagCount) + " gestartet</div>";
    if (stagStarted < stagCount) {
      if (stagStarted == 0) {
        html += "<div style='color:#4caf50;font-weight:bold'>&#9654; Ersten Fahrer starten</div>";
        ...
        unsigned long rem = (elapsed < off_ms) ? (off_ms - elapsed) : 0;
        if (rem > 0) {
          ...
          html += "<div style='color:#4caf50;font-weight:bold'>&#9654; Bereit!</div>";
        }
        html += "<div style='color:#555;font-size:.8em'>" + htmlEsc(stagRiders[stagStarted]) + "</div>";
      }
    } else {
      html += "<div style='color:#888;font-size:.85em'>Alle gestartet &mdash; warte auf Ergebnisse&hellip;</div>";
    }
    html += "<table style='margin-top:10px'><tr><th class='rk'>#</th><th>Fahrer</th><th>Status</th></tr>";
    for (uint8_t i = 0; i < stagCount; i++) {
      bool st = (i < stagStarted);
      bool fin = (stagFinishedMask & (1u << i)) != 0;
      String status;
      if (fin) { fmtTime(stagTimes[i], buf); status = "<span style='color:#4caf50'>" + String(buf) + "</span>"; }
      else if (st) status = "<span style='color:#ff9800'>l&auml;uft</span>";
      else status = "<span style='color:#555'>wartet</span>";
      html += "<tr><td class='rk'>" + String(i+1) + "</td><td>" + htmlEsc(stagRiders[i]) + "</td><td>" + status + "</td></tr>";
    }
    html += "</table>";
    html += "<a class='btn' style='margin-top:10px;background:#2a0a0a;color:#f44' href='/stagexit' onclick=\"return confirm('Abbrechen?')\">&#10006; Abbrechen</a></div>";
  } else {
    html += "<a class='dbtn' href='/stag'>&#9201; Versetzten Start einrichten</a>";
    html += "<div class='c'><div class='lb'>Info</div><div style='color:#555;font-size:.85em'>"
      "Fahrer starten im Versatz &bull; Offset: " + String(cfg_stag_offset_s) + " s</div></div>";
  }
  html += "</div>"; // msc-stag
