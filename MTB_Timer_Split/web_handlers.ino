...
void handleSettingsSave() {
  ...
  if (server.hasArg("maxretry"))
    cfg_max_retries = (uint8_t)constrain(server.arg("maxretry").toInt(), 1, 10);
...
void handleExport() {
  ...
  for (uint8_t rank = 0; rank < validCnt; rank++) {
    ...
    csv += String(rank + 1) + "," + String(history[i]) + "," + String(tbuf) + "," + String(dateBuf) + "\r\n";
  }
