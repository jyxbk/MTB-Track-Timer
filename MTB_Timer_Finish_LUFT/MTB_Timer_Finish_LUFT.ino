...
char     cfg_ap_ssid[33]          = "MTB-Timer-Ziel-L";
char     cfg_ap_pass[64]          = "";
...
void setup() {
  ...
  if (strlen(cfg_ap_ssid) == 0) strcpy(cfg_ap_ssid, "MTB-Timer-Ziel-L");
  ...
  u8g2.setFont(u8g2_font_6x10_tf);
  u8g2.drawStr(10, 32, "FINISH LUFT v2");
...
void loop() {
  ...
  if (plateFlag && appState == ARMED) {
    ...
    if (elapsedUs > (cfg_debounce_ms * 1000ULL)) {
      ...
      sprintf(txBuf, "TIM:%lu", (unsigned long)elapsedRaw);
      loRaSend(txBuf);
      retryCnt    = 0;
      ackReceived = false;
      lastRetryAt = now;
      ...
      DBGF("RESULT",  "%s  Fahrer: %s  (%luus roh)",
           tbuf,
