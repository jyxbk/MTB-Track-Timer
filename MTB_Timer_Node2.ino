...
//
//  Ablauf:
//    - Empfängt "STX" von Node 1 → startet interne Stoppuhr
//    - Druckplatte am Ziel → stoppt Stoppuhr, sendet "TIM:xxxxx"
//    - Empfängt "ACK" von Node 1 → kein weiterer Retry
//    - PRG-Taste (GPIO 0): 3 Sek. halten → Deep Sleep
//      Aufwachen: PRG-Taste kurz drücken
//    - WiFi Access Point "MTB-Timer-Ziel"
...
void drawDisplay(unsigned long liveMs = 0) {
  ...
  u8g2.drawStr(0, 10, "MTB TIMER   [ZIEL]");
...
String buildHTML() {
  ...
  html = "<!DOCTYPE html><html lang='de'><head>"
         ...
         ".s-idle{background:#1a1a3a;color:#888}"
         ".s-arm{background:#1a3a1a;color:#4caf50}"
         ".s-done{background:#1a2a3a;color:#2196f3}"
         "</style></head><body>";

  html += "<h1>&#9201; MTB TIMER</h1>";

  // Node + Status
  const char* stateStr   = (appState == IDLE) ? "WARTET" : (appState == ARMED) ? "LAEUFT" : "ERGEBNIS";
  const char* stateClass = (appState == IDLE) ? "s-idle" : (appState == ARMED) ? "s-arm"  : "s-done";
  html += "<div class='card'>";
  html += "<div class='lbl'>Node &nbsp; <span class='state " + String(stateClass) + "'>" + stateStr + "</span></div>";
  html += "<div class='val'>&#127937; ZIEL-NODE</div>";
  html += "</div>";
  ...
  html += "<div class='card'><div class='lbl'>LoRa Verbindung zum Start-Node</div>";
...
void setup() {
  ...
  u8g2.setFont(u8g2_font_6x10_tf);
  u8g2.drawStr(10, 32, "ZIEL NODE   v2");
...
void loop() {
  ...
  if (btnNow == LOW && btnPrev == HIGH) {
    ...
    if (held >= LONG_PRESS_MS) {
      ...
      showSleepProgress(held);
    }
  } else if (btnNow == HIGH && btnPrev == LOW) {
    if ((now - btnDownAt) < LONG_PRESS_MS) drawDisplay();
  }
  btnPrev = btnNow;
  ...
  if (rxFlag) {
    ...
    if (rc == RADIOLIB_ERR_NONE) {
      ...
      Serial.print("[Node2] RX: "); Serial.println(msg);
      ...
      if (msg == "STX") {
        ...
        appState    = ARMED;
        drawDisplay();
        Serial.println("[Node2] STX – bereit.");
      }
      if (msg == "ACK" && appState == DONE) {
        retryCnt = MAX_RETRIES;   // Keine weiteren Retries
        Serial.println("[Node2] ACK empfangen.");
      }
      if (msg == "PNG") {
        loRaSend("POG");          // Ping beantworten
        Serial.println("[Node2] PNG → POG.");
      }
    }
  }
  ...
  if (digitalRead(PLATE_PIN) == LOW
      ...
      && appState == ARMED) {
    lastTrigger           = now;
    unsigned long elapsed = now - startRecvAt;
    lastTimeMs = elapsed;
    if (bestTimeMs == 0 || elapsed < bestTimeMs) bestTimeMs = elapsed;
    addHistory(elapsed);
    sprintf(txBuf, "TIM:%lu", elapsed);
    loRaSend(txBuf);
    retryCnt    = 1;
    lastRetryAt = now;
    doneAt      = now;
    appState    = DONE;
    drawDisplay();
    char buf[12]; fmtTime(elapsed, buf);
    Serial.print("[Node2] ZIEL! Zeit: "); Serial.println(buf);
  }

  // ── Live-Stoppuhr ───────────────────────────────────────
  if (appState == ARMED && (now - lastDispRefr) >= DISP_REFRESH_MS) {
    lastDispRefr = now;
    drawDisplay(now - startRecvAt);
  }

  // ── Lauf-Timeout ────────────────────────────────────────
  if (appState == ARMED && (now - startRecvAt) > RUN_TIMEOUT_MS) {
    appState = IDLE;
    drawDisplay();
    Serial.println("[Node2] Timeout.");
  }

  // ── Ergebnis-Retries (bis ACK) ──────────────────────────
  if (appState == DONE
      && retryCnt < MAX_RETRIES
      && (now - lastRetryAt) >= RETRY_INTERVAL) {
    loRaSend(txBuf);
    lastRetryAt = now;
    retryCnt++;
    Serial.print("[Node2] Retry #"); Serial.println(retryCnt);
  }

  // ── Ergebnis-Anzeigedauer ───────────────────────────────
  if (appState == DONE && (now - doneAt) >= RESULT_SHOW_MS) {
    appState = IDLE;
    drawDisplay();
  }
}
