...
String buildHTML() {
  ...
  html = "<!DOCTYPE html><html lang='de'><head>"
    ...
    ".si{background:#1a1a2a;color:#888}.sr{background:#221500;color:#ff9800}.sd{background:#0d1525;color:#64b5f6}.se{background:#200d0d;color:#f44336}"
    ...
    "</style></head><body>"
    "<h1>&#9201; MTB ZIEL <span id='_ld' class='ld on'></span></h1>";
  ...
  html += "<div class='c'><div class='row'>"
    ...
    "<div id='sync-ind' style='font-size:.65em;color:#383838;margin-top:2px'>" +
    String(timeIsSynced && (millis() - lastSyncAt) < 300000 ? "&#10003; Uhr sync" : "&#9888; Uhr nicht sync") +
    "</div></div>"
  ...
  html += "<script>"
    ...
    "var last={hist:_lastHist,bestMs:_lastBest,histCnt:_lastHist?_lastHist.length:0};"
    ...
    "if(rw){rw.style.display=d.rider?'block':'none';document.getElementById('rider').textContent=d.rider||'';}"
