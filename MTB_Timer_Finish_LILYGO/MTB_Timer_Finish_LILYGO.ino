// ============================================================
//  FINISH-NODE  v11  –  LILYGO TTGO T3 V1.6.1 (ESP32 + SX1276)
...
char     cfg_ap_ssid[33]    = "MTB-Ziel-LILYGO";
char     cfg_ap_pass[64]    = "";
...
static const char* SD_LOGFILE = "/ziel_log.csv";
...
IRAM_ATTR void onPlateTrigger() {
  if (!plateFlag) { plateTriggerUs = (uint64_t)esp_timer_get_time(); plateFlag = true; }
}
...
void fmtTime(unsigned long ms, char* out) {
  sprintf(out, "%02u:%02u.%03u",
    (unsigned)(ms / 60000), (unsigned)((ms % 60000) / 1000), (unsigned)(ms % 1000));
}
...
uint8_t voltageToPct(float v) {
  ...
  for (int i = 1; i < 7; i++) {
    if (v < vpts[i]) return (uint8_t)(ppts[i-1]+(v-vpts[i-1])/(vpts[i]-vpts[i-1])*(ppts[i]-ppts[i-1]));
  }
  return 0;
}
...
void updateLED(unsigned long now) {
  ...
  if (ledBlinkCount > 0) {
    if (now - ledBlinkAt >= 50) { ledBlinkAt=now; ledBlinkCount--; ledOn=(ledBlinkCount%2!=0); digitalWrite(LED_PIN,ledOn?HIGH:LOW); } return;
  }
  switch (appState) {
...
void goToSleep() {
  ...
  u8g2.clearBuffer(); u8g2.setFont(u8g2_font_7x13B_tf);
  u8g2.drawStr(22,35,"Schlafe..."); u8g2.sendBuffer();
  delay(800); u8g2.setPowerSave(1); SD.end();
  radio.sleep(); WiFi.softAPdisconnect(true); WiFi.mode(WIFI_OFF);
...
// ── Cancel ─────────────────────────────────────────────────
void cancelRun() {
  appState = IDLE; plateFlag = false; currentRiderName[0] = '\0'; currentPage = 0;
  drawDisplay(); DBG("STATE", "IDLE ← Lauf abgebrochen");
}
...
void initDisplay() {
  ...
  u8g2.begin(); u8g2.setContrast(cfg_contrast);
}

// ═══════════════════════════════════════════════════════════
void setup() {
  Serial.begin(115200);
  prefs.begin("mtb-cfg2-l", true);
  ...
  prefs.end();
  if (strlen(cfg_ap_ssid) == 0) strcpy(cfg_ap_ssid, "MTB-Ziel-LILYGO");
  ...
  if (rc != RADIOLIB_ERR_NONE) {
    u8g2.clearBuffer(); char err[24]; sprintf(err,"LoRa Fehler: %d",rc); u8g2.drawStr(0,35,err); u8g2.sendBuffer();
  ...
  u8g2.setFont(u8g2_font_6x10_tf); u8g2.drawStr(4,32,"FINISH LILYGO v11");
...
void loop() {
  ...
  else if (btnNow == LOW) { unsigned long held=now-btnDownAt; if(held>=LONG_PRESS_MS)goToSleep(); else if(held>=1000)showSleepProgress(held); }
  ...
  if (rxFlag) {
    ...
    if (rc == RADIOLIB_ERR_NONE) {
      ...
      if (msg.startsWith("STX")) {
        if (msg.length()>4&&msg[3]==':') { strncpy(currentRiderName,msg.c_str()+4,NAME_MAX_LEN); currentRiderName[NAME_MAX_LEN]='\0'; }
        else currentRiderName[0]='\0';
        startRecvUs=(uint64_t)esp_timer_get_time(); plateFlag=false; retryCnt=0; ackReceived=false; appState=ARMED;
        drawDisplay();
        DBGF("STATE", "ARMED ← STX  Fahrer: %s", strlen(currentRiderName)>0?currentRiderName:"(kein)");
      }
      if (msg=="ACK"&&appState==DONE) { ackReceived=true; DBG("LORA-RX","ACK – Messung bestätigt"); }
      if (msg=="PNG") { loRaSend("POG"); DBGV("LORA-RX","PNG → POG gesendet"); }
      ...
      if (msg=="CAN"&&appState==ARMED) { cancelRun(); DBG("STATE","IDLE ← CAN empfangen"); }
    }
  }
  ...
  if (plateFlag && appState == ARMED) {
    ...
    if (elapsedUs > (cfg_debounce_ms * 1000ULL)) {
      ...
      loRaSend(txBuf); retryCnt=0; ackReceived=false; lastRetryAt=now; doneAt=now; appState=DONE;
      drawDisplay();
      char tbuf[12]; fmtTime(elapsedMs,tbuf);
      bool isBestF = (bestTimeMs > 0 && elapsedMs == bestTimeMs);
      DBGF("RESULT", "%s  Fahrer: %s%s  (%luus roh)",
           tbuf, strlen(currentRiderName)>0?currentRiderName:"(kein)",
           isBestF?"  *** BESTZEIT ***":"", (unsigned long)elapsedRaw);
    }
  }
  ...
  if (appState == ARMED) {
    ...
    if ((curUs - startRecvUs) / 1000 > cfg_run_timeout_ms) {
      appState=IDLE; currentRiderName[0]='\0'; drawDisplay();
  ...
  if (now - lastHbAt >= 25000) {
    lastHbAt = now;
    if (appState == IDLE) { loRaSend("HBT"); DBGV("HBT","HBT gesendet"); }
  }
