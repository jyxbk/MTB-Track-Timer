...
void handleDuelCount() {
  ...
  if (n < 1) n = 1; if (n > DUEL_MAX_RIDERS) n = DUEL_MAX_RIDERS;
  duelSetupCount = (uint8_t)n; duelSetupStep = 0; duelCount = 0;
  memset(duelRiders, 0, sizeof(duelRiders));
  server.sendHeader("Location", "/duelname"); server.send(303);
}
...
void handleDuelName() {
  ...
  html += "<h1>&#127944; Duell-Modus</h1><div class='sub'>Schritt 2 von 3 &ndash; Fahrer " +
    String(duelSetupStep + 1) + " von " + String(duelSetupCount) + "</div>";
  html += "<div class='prog'><div class='progb' style='width:" + String(pct) + "%'></div></div>";
  html += "<form action='/duelnext' method='GET'><div class='lb'>Name</div>"
    ...
    "<div class='row'><a class='back' href='/duel'>&#8592; Neu starten</a>"
...
void handleDuelNext() {
  ...
  duelSetupStep++;
  server.sendHeader("Location", duelSetupStep < duelSetupCount ? "/duelname" : "/duelconfirm");
  server.send(303);
}
...
void handleDuelConfirm() {
  ...
  html += "</ul><div class='row'><a class='back' href='/duel'>&#8592; Neu starten</a>"
    "<a class='ok' href='/duelgo'>&#9658; Starten</a></div></body></html>";
  server.send(200, "text/html", html);
}

void handleDuelGo() {
  duelMode = true; duelDone = false; duelCurrent = 0;
  duelCount = duelSetupCount; duelStartIdx = histHead;
  server.sendHeader("Location", "/"); server.send(303);
}

void handleDuelExit() {
  duelMode = duelDone = false;
  duelCount = duelCurrent = duelStartIdx = duelSetupCount = duelSetupStep = 0;
  server.sendHeader("Location", "/"); server.send(303);
}
...
void handleLapStart() {
  if (appState == IDLE && !duelMode && !duelDone) {
    lapMode = true; lapRoundNum = 0; lapRoundStart = 0;
    lapLastMs = 0; lapBestMs = 0; plateFlag = false;
    appState = LAP_IDLE; drawDisplay();
...
void handleLapStop() {
  if (appState == LAP_RUNNING) {
    ...
    lapLastMs = lapMs; lastTimeMs = lapMs;
    if (bestTimeMs == 0 || lapMs < bestTimeMs) bestTimeMs = lapMs;
    lapMode = false; appState = RESULT; resultAt = millis(); currentPage = 0;
    drawDisplay();
  } else if (appState == LAP_IDLE) {
    lapMode = false; appState = IDLE; drawDisplay();
  }
  server.sendHeader("Location", "/"); server.send(303);
}
...
void handleLapReset() {
  lapMode = true; lapRoundNum = 0; lapRoundStart = 0;
  lapLastMs = 0; lapBestMs = 0; bestTimeMs = 0; historyCnt = 0; histHead = 0; plateFlag = false;
  memset(history, 0, sizeof(history));
  memset(historyNames, 0, sizeof(historyNames));
  memset(historyTimestamp, 0, sizeof(historyTimestamp));
  sdClear(); appState = LAP_IDLE; drawDisplay();
  server.sendHeader("Location", "/"); server.send(303);
}
...
void saveSettings() {
  ...
  prefs.putUInt("ping",      cfg_ping_ms);
  ...
  if (prefs.getUChar("condition", 0) != cfg_condition) prefs.putUChar("condition", cfg_condition);
...
void handleSettingsSave() {
  ...
  if (server.hasArg("apssid")) {
    ...
    if (s.length() > 0 && s != String(cfg_ap_ssid)) {
      s.toCharArray(cfg_ap_ssid, sizeof(cfg_ap_ssid)); needsRestart = true;
    }
  }
  if (server.hasArg("appass")) {
    String pass = server.arg("appass");
    if ((pass.length() == 0 || pass.length() >= 8) && pass != String(cfg_ap_pass)) {
      pass.toCharArray(cfg_ap_pass, sizeof(cfg_ap_pass)); needsRestart = true;
    }
  }
  saveSettings();
...
void handleExport() {
  // SD bevorzugt, Fallback: RTC-Buffer
  if (sdPresent) {
    String csv = sdReadCsv();
    if (csv.length() > 0) {
      server.sendHeader("Content-Disposition", "attachment; filename=\"start_history_sd.csv\"");
...
void handleSdClearAll() {
  if (!sdPresent) { server.send(200, "application/json", "{\"ok\":false}"); return; }
  sdClearAll();
  server.send(200, "application/json", "{\"ok\":true}");
}

// ── JSON-String-Escape (nur " und \) ─────────────────────
...
// ── NVS-Hilfsfunktion: Tracking-Felder speichern ──────────
static void saveTrackingNvs() {
  prefs.begin("mtb-cfg-l", false);
  if (prefs.getUChar("trackid",   0) != cfg_track_id)   prefs.putUChar("trackid",   cfg_track_id);
  if (prefs.getUChar("riderid",   0) != cfg_rider_id)   prefs.putUChar("riderid",   cfg_rider_id);
  if (prefs.getUChar("condition", 0) != cfg_condition)  prefs.putUChar("condition", cfg_condition);
  if (prefs.getUChar("sessionnum",1) != cfg_session_num)prefs.putUChar("sessionnum",cfg_session_num);
  if (prefs.getUInt("sessionday", 0) != cfg_session_day)prefs.putUInt("sessionday", cfg_session_day);
  prefs.end();
}
...
void handleTracks() {
  ...
  for (uint8_t i = 0; i < trackCount; i++) {
    if (i) j += ",";
    char tbuf[12]; fmtTime(trackList[i].best_ms, tbuf);
    j += "{\"id\":"; j += trackList[i].id;
    j += ",\"name\":\""; j += jsonEsc(trackList[i].name); j += "\"";
    j += ",\"info\":\""; j += jsonEsc(trackList[i].info); j += "\"";
    j += ",\"best\":"; j += trackList[i].best_ms;
    j += ",\"bestFmt\":\""; j += (trackList[i].best_ms > 0 ? tbuf : "--:--.---"); j += "\"";
    j += ",\"active\":"; j += (trackList[i].id == cfg_track_id ? "true" : "false");
    j += "}";
  }
  j += "]";
...
void handleTrackSave() {
  if (!sdPresent) { server.send(400, "application/json", "{\"ok\":false}"); return; }
  String name = server.arg("name"); name.trim();
  if (name.length() == 0 || name.length() > 20) { server.send(400, "application/json", "{\"ok\":false,\"msg\":\"Name 1-20 Zeichen\"}"); return; }
  String info = server.arg("info"); if (info.length() > 40) info = info.substring(0, 40);
  uint8_t id = (uint8_t)server.arg("id").toInt();

  if (id == 0) {
    // Neue Strecke: neue ID vergeben
    if (trackCount >= MAX_TRACKS) { server.send(400, "application/json", "{\"ok\":false,\"msg\":\"Max 10 Strecken\"}"); return; }
    id = 1;
    for (uint8_t i = 0; i < trackCount; i++) if (trackList[i].id >= id) id = trackList[i].id + 1;
    Track& tr = trackList[trackCount++];
    tr.id = id; tr.best_ms = 0;
    name.toCharArray(tr.name, sizeof(tr.name));
    info.toCharArray(tr.info, sizeof(tr.info));
  } else {
    // Bestehende Strecke updaten
    Track* tr = sdFindTrack(id);
    if (!tr) { server.send(400, "application/json", "{\"ok\":false}"); return; }
    name.toCharArray(tr->name, sizeof(tr->name));
    info.toCharArray(tr->info, sizeof(tr->info));
    if (cfg_track_id == id) strncpy(activeTrackName, tr->name, sizeof(activeTrackName));
  }
  sdSaveTracks();
  String r = "{\"ok\":true,\"id\":"; r += id; r += "}";
...
void handleTrackDel() {
  ...
  for (uint8_t i = 0; i < trackCount; i++) {
    if (trackList[i].id == id) {
      memmove(&trackList[i], &trackList[i + 1], (trackCount - i - 1) * sizeof(Track));
      trackCount--;
      if (cfg_track_id == id) { cfg_track_id = 0; activeTrackName[0] = '\0'; saveTrackingNvs(); }
      sdSaveTracks();
...
void handleRiders() {
  ...
  for (uint8_t i = 0; i < riderCount; i++) {
    if (i) j += ",";
    j += "{\"id\":"; j += riderList[i].id;
    j += ",\"name\":\""; j += jsonEsc(riderList[i].name); j += "\"";
    j += ",\"active\":"; j += (riderList[i].id == cfg_rider_id ? "true" : "false");
    // Bestzeit auf aktiver Strecke
    if (cfg_track_id > 0 && cfg_track_id <= MAX_TRACKS) {
      unsigned long b = riderList[i].bests[cfg_track_id - 1];
      char tbuf[12]; fmtTime(b, tbuf);
      j += ",\"best\":"; j += b;
      j += ",\"bestFmt\":\""; j += (b > 0 ? tbuf : "--:--.---"); j += "\"";
    }
    j += "}";
  }
  j += "]";
...
void handleRiderSave() {
  ...
  if (id == 0) {
    if (riderCount >= MAX_RIDERS) { server.send(400, "application/json", "{\"ok\":false,\"msg\":\"Max 10 Fahrer\"}"); return; }
    id = 1;
    for (uint8_t i = 0; i < riderCount; i++) if (riderList[i].id >= id) id = riderList[i].id + 1;
    Rider& rd = riderList[riderCount++];
    memset(&rd, 0, sizeof(Rider));
    rd.id = id;
    name.toCharArray(rd.name, sizeof(rd.name));
  } else {
    Rider* rd = sdFindRider(id);
    if (!rd) { server.send(400, "application/json", "{\"ok\":false}"); return; }
    name.toCharArray(rd->name, sizeof(rd->name));
    if (cfg_rider_id == id) strncpy(activeRiderName, rd->name, sizeof(activeRiderName));
  }
  sdSaveRiders();
  String r = "{\"ok\":true,\"id\":"; r += id; r += "}";
  server.send(200, "application/json", r);
...
void handleRiderDel() {
  ...
  uint8_t id = (uint8_t)server.arg("id").toInt();
  for (uint8_t i = 0; i < riderCount; i++) {
    if (riderList[i].id == id) {
      memmove(&riderList[i], &riderList[i + 1], (riderCount - i - 1) * sizeof(Rider));
      riderCount--;
      if (cfg_rider_id == id) { cfg_rider_id = 0; activeRiderName[0] = '\0'; saveTrackingNvs(); }
      sdSaveRiders();
      server.send(200, "application/json", "{\"ok\":true}"); return;
    }
  }
...
void handleCondition() {
  ...
  cfg_condition = (uint8_t)constrain(c, 0, 4);
  saveTrackingNvs();
  server.send(200, "application/json", "{\"ok\":true}");
}

// ── Fahrernamen editieren (RTC + SD) ─────────────────────
void handleEditName() {
  int idx = server.arg("idx").toInt();
  String name = server.arg("name"); name.trim();
  if (idx < 0 || idx >= historyCnt || name.length() == 0 || name.length() > NAME_MAX_LEN) {
    server.send(400, "application/json", "{\"ok\":false}"); return;
  }
  uint8_t pi = histPhys((uint8_t)idx);
  // RTC aktualisieren
  name.toCharArray(historyNames[pi], NAME_MAX_LEN + 1);
  // SD aktualisieren
  bool sdOk = sdEditLastEntry(historyTimestamp[pi], historyNames[pi]);
  String r = "{\"ok\":true,\"sdUpdated\":"; r += (sdOk ? "true" : "false"); r += "}";
  server.send(200, "application/json", r);
}

// ── Session-Daten ─────────────────────────────────────────
void handleSessions() {
  if (!sdPresent) { server.send(200, "application/json", "[]"); return; }
  server.send(200, "application/json", sdListSessions());
}

void handleSessionData() {
  if (!sdPresent) { server.send(200, "application/json", "[]"); return; }
  String fname = server.arg("f");
  uint8_t page = (uint8_t)server.arg("p").toInt();
  // Sicherheit: nur Dateinamen aus /logs erlauben
  if (fname.indexOf('/') >= 0 || fname.indexOf('.') != fname.lastIndexOf('.') || !fname.endsWith(".csv")) {
    server.send(400, "application/json", "[]"); return;
  }
  server.send(200, "application/json", sdReadSession(fname.c_str(), page));
}

// ── SD-Statistiken ────────────────────────────────────────
void handleSdStats() {
  if (!sdPresent) { server.send(200, "application/json", "{\"total\":0}"); return; }
  uint8_t id = (uint8_t)server.arg("id").toInt();
  server.send(200, "application/json", sdGetStats(id));
}
...
void handleBestlist() {
  if (!sdPresent) { server.send(200, "application/json", "[]"); return; }
  uint8_t id = (uint8_t)server.arg("id").toInt();
  server.send(200, "application/json", sdGetBestlist(id));
}
